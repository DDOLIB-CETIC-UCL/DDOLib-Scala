package be.cetic.ddolibscala.layered.util.testbench

import be.cetic.ddolibscala.layered.solver.Solution
import be.cetic.ddolibscala.layered.modeling.{DefaultFastLowerBound, Problem}
  ...
  def generateTests(): List[GeneratedTest] = {
    problems.flatMap { p =>
      ...
      add("Transition Model") {
        val solver = Solvers.layered.exact(p, debugMode = On)
        assertSolution(solver.minimize(), p)
      }

      if (!config.flb.isInstanceOf[DefaultFastLowerBound[?]]) {
        add("FLB") {
          val solver = Solvers.layered.exact(p, lowerBound = config.flb, debugMode = On)
          assertSolution(solver.minimize(), p)
        }
      }
      ...
      if (!config.dominance.isInstanceOf[DefaultDominanceChecker[?]]) {
        add("Dominance") {
          val solver = Solvers.layered.exact(p, dominance = config.dominance, debugMode = On)
          assertSolution(solver.minimize(), p)
        }
      }
      ...
      config.relaxation.foreach { relax =>
        add("Relaxation") {
          for (w <- minWidth to maxWidth) {
            val solver = Solvers.layered.ddo(
              p,
              relaxation = relax,
      ...
      add("A*") {
        val solver =
          Solvers.layered.astar(p, lowerBound = config.flb, dominance = config.dominance, debugMode = On)
        assertSolution(solver.minimize(), p)
      }

      add("ACS") {
        val solver =
          Solvers.layered.acs(p, lowerBound = config.flb, dominance = config.dominance, debugMode = On)
        assertSolution(solver.minimize(), p)
      }

      add("AWA*") {
        val solver =
          Solvers.layered.awastar(p, lowerBound = config.flb, dominance = config.dominance, debugMode = On)
        assertSolution(solver.minimize(), p)
      }
